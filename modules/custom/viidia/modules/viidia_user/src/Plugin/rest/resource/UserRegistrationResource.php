 ...
 *
 * @RestResource(
 *   id = "viidia_user_registration_rest",
 *   label = @Translation("Viidia User registration REST"),
 *   serialization_class = "Drupal\user\Entity\User",
 *   uri_paths = {
 *     "https://www.drupal.org/link-relations/create" = "/viidia_user/register",
 *   },
 * )
 */
class UserRegistrationResource extends ResourceBase
{
    ...
    public function __construct(array $configuration, $plugin_id, $plugin_definition, array $serializer_formats,
        LoggerInterface $logger, ImmutableConfig $user_settings, AccountInterface $current_user, Request $current_request)
    {
    ...
    {
        $regCode = $this->currentRequest->get('reg_code');
        if (!empty($regCode)) {
            ...
            if ($invite) {
                ...
                throw new BadRequestHttpException('Invalid invitation code.');
            }
            $account->activate();
        ...
        if (!empty($regCode)) {
            ...
            $invite->setJoined(\Drupal::time()->getCurrentTime());
